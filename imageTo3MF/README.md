...
Use `uv run python image_grade_to_3mf.py /path/to/image.png --thickness 1mm --width 100mm --height 100mm --resolution 0.25mm --lead-thickness 0.25mm --lead-cap-height 0.3mm --plate-width 270mm --plate-height 270mm --description "cartoon spaniel"` to convert an image into segmented color zones plus black separator lines, export the result as a multi-object `.3mf`, and open it in Snapmaker Orca.
...
If `--description` is supplied, the default output names become `out/<input-stem>_<description>_graded.3mf` and `out/<input-stem>_<description>_preview.png`, using a lowercase filename-safe version of the description.
