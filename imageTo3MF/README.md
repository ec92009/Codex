...
There are also launcher scripts at `~/Codex/imageTo3MF/launch_leadlight_gui.sh` and `~/Codex/imageTo3MF/launch_leadlight_gui.command`.

For a no-Terminal macOS launcher, build the local app bundle with `~/Codex/imageTo3MF/build_leadlight_app.sh`. That creates `~/Codex/imageTo3MF/dist/LeadLight.app`.

If you switch computers, `git pull` in `~/Codex` first, then rerun `~/Codex/imageTo3MF/build_leadlight_app.sh` on that machine so the local bundle matches the latest code.
